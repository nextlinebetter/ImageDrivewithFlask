<template>
  <el-card>
    <div class="title">
      <el-icon><Picture /></el-icon><span>以图搜图</span>
    </div>
    <el-input
      v-model.number="imageId"
      placeholder="输入一个已有的 image_id（可用上传返回值）"
    ...
    <el-empty
      ...
      description="无结果，请确认 image_id 是否存在或多上传几张"
    ...
    <el-table :data="rows" v-if="rows.length" style="margin-top: 16px">
      ...
      <el-table-column label="Similarity" width="220">
        <template #default="{ row }">
          <el-progress
            :percentage="Math.round((row.similarity || 0) * 100)"
            :text-inside="true"
            :stroke-width="16"
            status="success"
          />
...
async function onSearch() {
  ...
  loading.value = true;
  try {
    const { data } = await api.get(`/search/image/${imageId.value}/similar`, {
      params: { top_k: 10 },
    });
