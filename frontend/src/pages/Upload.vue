<template>
  <el-card>
    ...
    <el-result v-if="resp && resp.code === 0" icon="success" title="上传成功">
      <template #sub-title>
        <el-descriptions :column="1" size="small" border>
          <el-descriptions-item label="image_id">{{
            resp.data.image_id
          }}</el-descriptions-item>
          <el-descriptions-item label="has_embedding">{{
            String(resp.data.has_embedding)
          }}</el-descriptions-item>
          <el-descriptions-item label="has_ocr_text">{{
            String(resp.data.has_ocr_text)
          }}</el-descriptions-item>
        </el-descriptions>
        <div style="margin-top: 12px">
          <el-button type="primary" @click="goSimilar(resp.data.image_id)"
            >查看相似</el-button
          >
    ...
    <el-alert
      v-else-if="resp && resp.code !== 0"
      type="error"
      :title="resp.message || '上传失败'"
      show-icon
    />
  </el-card>
</template>
